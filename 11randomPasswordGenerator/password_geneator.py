...
class Application(tk.Frame):
	...
	def create_checkbuttons(self):
		...
		for option in self.password_options:
			...
			self.chkbtn = tk.Checkbutton(self, text=option, variable=self.password_options[option])
			self.chkbtn.select()
			self.chkbtn.pack()
	...
	def create_combobox(self):
		...
		self.password_length = tk.IntVar(self)
		self.password_length.set(4) # initial value
	...
	def create_password_label(self):
		...
		self.pass_lbl = tk.Label(self, textvariable=self.your_password)
		self.pass_lbl.pack()

	def create_button(self):
		self.btn = tk.Button(self, text='btn', command=self.generate_password)
		self.btn.pack()
